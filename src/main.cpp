...
int main(int argc, char* argv[])
{
    ...
    std::filesystem::path dataPath = exeDir.parent_path() / "assets";
    ...
    if (!std::filesystem::exists(dataPath) && std::filesystem::exists(exeDir.parent_path() / "data")) {
        dataPath = exeDir.parent_path() / "assets";
    }
    ...
    if (std::filesystem::exists(dataPath)) {
        int file_count = 0;
        for (auto const &file : std::filesystem::directory_iterator(dataPath)) {
